...
module.exports = class Element {
  ...
  pug() {
    let classes = (this.class())            ? `.${this.class().split(' ').join('.')}` : ''
    let id      = (this.id())               ? `#${this.id()}`                         : ''
    let attrs   = (this._attributeString()) ? `(${this._attributeString()})`          : ''
    return `${this.name}${id}${classes}${attrs}${
        (!this.isVoid) ? `
  ${Element.parse(this.contents).pug()}` : ''
      }`
  }
}
